...
int __init initialize()
{
	...
	for (i = 0; i<MAX_NUM_CHANNELS; i++) {
        ...
        if (status != 0) {
			printk("%s: ALERT: Send Failed to initialize pcie connection: %d\n", __func__, status);
			return;
		}
        ...
        if (status != 0) {
			printk("%s: ALERT: Send Failed to dma_init: %d\n", __func__, status);
			return;
        }
#endif
	
	}
    /* recevier thread pool */
	for (i = 0; i<MAX_NUM_CHANNELS; i++) {
...
int test_thread(void* arg0)
{
	...
	for (i = 0; i<(NUM_MSGS); i++) {
		...
		pci_kmsg_send_long(1,(struct pcn_kmsg_long_message*)msg, payload_size);
                ends = ktime_get();
                int internal = (ktime_to_ns(ktime_sub(ends,starts)))/1000;
                printk("send internal time = %lld  round: %d\n", internal, i);
	}
...
int send_process(send_wait *send_data, int channel_num)
{
        ...
        printk("In MSG LAYER send process for channel %d\n", channel_num);
        ...
        status = dis_start_dma_transfer(subuser_id[channel_num], send_vaddr[channel_num], local_io[channel_num], pcn_msg->hdr.size, 0, remote_recv_seg_hdl[channel_num], dma_cb, &dma_queue[channel_num], &dma_queue[channel_num], DMA_PUSH);
        if (status != 0) {
            printk(" %s: Error in dis_start_dma_transfer: %d\n", __func__, status);
			return;
        }
...
//	send_process(send_data, channel_select);
#else
	sendstart = ktime_get();
    send_process(send_data, 0);
    sendend   = ktime_get();
	int tcon = (ktime_to_ns(ktime_sub(end,start)))/1000;
	printk("send process time: %d\n", tcon); 
#endif
