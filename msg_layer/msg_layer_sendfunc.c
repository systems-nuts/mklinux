...
ktime_t recvstart[25], recvend[25];
static int recvi=0;

ktime_t sendstart[25], sendend[25];
static int sendi=0;

ktime_t readstart[25], readend[25];
static int readi=0;

ktime_t writestart[25], writeend[25];
static int writei=0;

ktime_t pcistart[25], pciend[25];
static int pcii=0;

ktime_t relstart[25], relend[25];
static int reli=0;

ktime_t rtrigstart[25], rtrigend[25];
static int rtrii=0;

ktime_t allrstart[25], allrend[25];
static int alli=0;


ktime_t ttrigstart[25], ttrigend[25];
static int ttri=0;
...
int __init initialize()
{
	...
	for (i = 0; i<MAX_NUM_CHANNELS; i++) {
	    ...
	    if (status != 0) {
		    printk(" Failed to initialize Send pcie connection\n");
	    }	
		up(&send_connDone[i]);
	    printk("%s: INFO: Connection Done...PCN_SEND \n", __func__);
	}
...
int send_process(send_wait *send_data, int channel_num)
{
    ...
    sendstart[sendi]=ktime_get();
	wait_for_completion(&send_intr_flag[channel_num]);
    sendend[sendi]=ktime_get();
    sendi++;
        ...
        writestart[writei] = ktime_get();
		memcpy(send_remote_vaddr[channel_num], pcn_msg, pcn_msg->hdr.size);
        writeend[writei] = ktime_get();
        writei++;
#endif

		ttrigstart[ttri] = ktime_get();	
     ...
     //           printk("recv triggered interrupt number: %d\n", remote_send_intr_hdl[channel_num]);
		...
		if (pcn_msg->hdr.type < 0) {
			...
			if (ftn != NULL) {
                ...
                relend[reli]=ktime_get();
		reli++;
                arend[ari] = ktime_get();
                ari++;
                int arage = (ktime_to_ns(ktime_sub(arend[ari-1],arstart[ari-1])));
                printk("recv side time = %lld ns, round = %d\n", arage, ari);
#if TEST_MSG_LAYER
...
static int pcie_send_init(int channel_num)
{
	...
	if (send_vaddr != NULL)
	{
		printk(" send  local segment kernel virtual address is: %p\n", send_vaddr[channel_num]);
	}
	...
	if (status == 0)
	{
		printk("Send Local segment not available to connect to\n");
	}
...
static int pcie_recv_init(int channel_num)
{
	...
	if (send_vaddr != NULL)
	{
	        printk("Recv local segment kernel virtual address is: %x\n", recv_vaddr);
	}
	...
	if (status == 0)
	{
		printk(" recv Local segment not available to connect to, status: %d\n", status);
	}
...
static int pcie_recv_cleanup(int channel_num)
{
	...
	status = sci_unbind (&recv_binding[channel_num]);
	if (status != 0) {
...
static void __exit unload(void)
{
	...
	sci_terminate(module_id);	
		...
		average = (ktime_to_ns(ktime_sub(allrend[f],allrstart[f])));
                ...
                average = (ktime_to_ns(ktime_sub(arend[f],arstart[f])));
                printk("recv side time = %lld ns, round = %d\n", average, f+1);

		#endif 

                #if TEST_SERVER	
                int average = (ktime_to_ns(ktime_sub(sendend[f],sendstart[f])));
